...
class Vendors extends Component {
  ...
  onLearnMore = (vendor) => {
    this.props.navigation.navigate('VendorDetail', { ...vendor });
  };
      ...
      return (
        ...
        <ScrollView style={styles.defaultContainer}>
          ...
          <View style={styles.altWithPadding}>
            <Text key={"ticket header"} style={styles.altHeaderText}>{"Amazing Options"}</Text>
            <Text key={"ticket description"} style={styles.altText}>{"CaperCon hosts some of the best pop culture vendors in Nova Scotia."}</Text>
          </View>
              ...
              <ListItem
                ...
                subtitle={
                  <View style={styles.subtitleView}>
                    <Image source={{ uri: validatePhotoUrl(vendor.photo_url) }} style={styles.subtitleImage}/>
                    <Text style={styles.subtitleText}>{vendor.company}</Text>
                  </View>
                }
                onPress={() => this.onLearnMore({name: vendor.company, description: vendor.promotion, photo: vendor.photo_url, twitter: vendor.twitter, facebook: vendor.facebook, website: vendor.website})}
                underlayColor={secondaryBGColour}
                chevronColor={secondaryHighlightColour}
              />
            ))}

            {this.state.Data2.map((vendor) => (
              <ListItem
                key={vendor.slug}
                style={styles.featuredExhibitor}
                subtitle={
